...
void MAVConnInterface::parse_buffer(const char *pfx, uint8_t *buf, const size_t bufsize, size_t bytes_received)
{
	...
	if(hex_received_cb)
	{
		hex_received_cb(buf,bytes_received);
	}
}
		...
		bool is_multicast)
{
	...
	return std::make_shared<MAVConnUDP>(system_id, component_id,
			bind_host, bind_port,
			remote_host, remote_port,is_multicast);
}
		...
		uint8_t component_id)
{
	...
	if (proto == "udp")
		interface_ptr = url_parse_udp(host, query, system_id, component_id, false, false,false);
	else if (proto == "udp-b")
		interface_ptr = url_parse_udp(host, query, system_id, component_id, true, false,false);
	else if (proto == "udp-pb")
		interface_ptr = url_parse_udp(host, query, system_id, component_id, true, true,false);
	else if (proto == "udp-m")
		interface_ptr = url_parse_udp(host, query, system_id, component_id, false, false,true);
	else if (proto == "tcp")
