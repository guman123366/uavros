...
MAVConnUDPMulticast::MAVConnUDPMulticast(uint8_t system_id, uint8_t component_id,
		std::string bind_host, unsigned short bind_port,
		std::string remote_host, unsigned short remote_port,
		bool multicast_state) :
	...
	tx_q {},
	rx_buf {}
	is_multicast(multicast_state)
{
	using udps = boost::asio::ip::udp::socket;

	bind_ep=endpoint(udp::v4(),bind_port);
...
MAVConnUDPMulticast::~MAVConnUDPMulticast()
{
	close();
}

void MAVConnUDPMulticast::connect(
		...
		const ClosedCb &cb_handle_closed_port)
{
	...
	io_service.post(std::bind(&MAVConnUDPMulticast::do_recvfrom, this));
...
void MAVConnUDPMulticast::connectHex(
		const ReceiveHexCb &cb_handle_message,
		const ClosedCb &cb_handle_closed_port)
{
	//message_received_cb = cb_handle_message;
	hex_received_cb=cb_handle_message;
	port_closed_cb = cb_handle_closed_port;
...
void MAVConnUDPMulticast::close()
{
...
void MAVConnUDPMulticast::send_bytes(const uint8_t *bytes, size_t length)
{
	if (!is_open()) {
...

	{
		...
		if (tx_q.size() >= MAX_TXQ_SIZE)
			throw std::length_error("MAVConnUDPMulticast::send_bytes: TX queue overflow");

		tx_q.emplace_back(bytes, length);
	}
	io_service.post(std::bind(&MAVConnUDPMulticast::do_sendto, shared_from_this(), true));
}

void MAVConnUDPMulticast::send_message(const mavlink_message_t *message)
{
...

	{
		...
		if (tx_q.size() >= MAX_TXQ_SIZE)
			throw std::length_error("MAVConnUDPMulticast::send_message: TX queue overflow");
...
void MAVConnUDPMulticast::send_message(const mavlink::Message &message, const uint8_t source_compid)
{
	if (!is_open()) {
		CONSOLE_BRIDGE_logError(PFXd "send: channel closed!", conn_id);
		return;
...
void MAVConnUDPMulticast::do_recvfrom()
{
...
void MAVConnUDPMulticast::do_sendto(bool check_tx_state)
{
...
std::string MAVConnUDPMulticast::get_remote_endpoint() const
{
