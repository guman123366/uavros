	...
	is_multicast(multicast_state)
{
	...
	if (remote_host != "") {
		...
		{
			remote_exists = true;
			remote_ep=boost::asio::ip::udp::endpoint(boost::asio::ip::address_v4::from_string(remote_host),remote_port);
			bind_ep=boost::asio::ip::udp::endpoint(boost::asio::ip::address_v4::any(),bind_port);
		}
		else if (remote_host != BROADCAST_REMOTE_HOST && remote_host != PERMANENT_BROADCAST_REMOTE_HOST)
	...
	try {
		...
		if(is_multicast)
		{
			...
			boost::asio::ip::address address = boost::asio::ip::address::from_string(remote_host);
			
			// 尝试指定接口
			boost::asio::ip::address_v4 localInterface = boost::asio::ip::address_v4::from_string(bind_host); // 或者指定具体IP
			socket.set_option(
				boost::asio::ip::multicast::join_group(address.to_v4(), localInterface), ec);
			
			if (ec) {
				std::cout << "Join multicast failed: " << ec.message() << std::endl;
				// 尝试不指定接口
				socket.set_option(boost::asio::ip::multicast::join_group(address.to_v4()), ec);
				if (ec) {
					std::cout << "Second attempt also failed: " << ec.message() << std::endl;
				}
			} else {
				std::cout << "Joined multicast group successfully" << std::endl;
			}
		}
		else if (remote_host == BROADCAST_REMOTE_HOST) {
...
void MAVConnUDP::do_recvfrom()
{
	...
	bool b=is_multicast||permanent_broadcast;
