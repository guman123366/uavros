	...
	: DataAnalysisInterface()
{
	...
	m_TD550TelemetryData = std::make_shared<TD550TelemetryData>();
}
...
std::shared_ptr<DataDefineInterface> TD550DataAnalysis::AnalyseData(unsigned char* buf, int nLength)
{
					...
					if (g_rxBuf[0] == START1)//第一副帧原始数据，分为A机、B机
					{
						if (1 == 1)//450与101切换标识，暂时不使用
						{
							...
							if (g_rxBuf[1] == 0x21)
							{
								...
								secondOneFrame.MemsWarningState = (g_rxBuf[23] * 0x1000000 + g_rxBuf[22] * 0x10000 + g_rxBuf[21] * 0x100 + g_rxBuf[20])* 1.0 * 0.001;
							...
							if (g_rxBuf[1] == 0x31)
							{
								...
								thirdOneFrame.MemsTemp=(g_rxBuf[23]+g_rxBuf[43]*0x100)*1.0*0.01;
								thirdOneFrame.OpenK2 = (*(short*)&g_rxBuf[28])* 1.0 * 0.001;
								thirdOneFrame.OilPressure = (*(short*)&g_rxBuf[33])* 1.0 * 0.001;
								thirdOneFrame.OilVolume = (*(short*)&g_rxBuf[35])* 1.0 * 0.001;
								thirdOneFrame.OpenK4=(*(short*)&g_rxBuf[37])* 1.0 * 0.001;
								thirdOneFrame.RetarderPressure = (*(short*)&g_rxBuf[39])* 1.0 * 0.001;
								thirdOneFrame.RetarderTemp = (*(short*)&g_rxBuf[41])* 1.0 * 0.1;
								thirdOneFrame.OpenK1 = (*(short*)&g_rxBuf[58])* 1.0* 0.001;
								thirdOneFrame.Tension = (*(short*)&g_rxBuf[60])* 1.0 * 0.001;
								m_YCMutex.unlock();
							}
						}
					}
					else if (g_rxBuf[0] == START2)//第二副帧原始数据，分为A机、B机
					{
						if (1 == 1)//450与101切换标识，暂时不使用
						{
							if (g_rxBuf[1] == 0x12)
							{
								...
								firstTwoFrame.DamperServoPostion = (*(short*)&g_rxBuf[40])* 1.0 * 0.001;//前旋翼总距桨距角
								firstTwoFrame.DirServoPostion = (*(short*)&g_rxBuf[42])* 1.0 * 0.001;//前旋翼俯仰桨距角
							...
							if (g_rxBuf[1] == 0x22)
							{
								...
								secondTwoFrame.chk = (*(short*)&g_rxBuf[23])* 1.0 * 0.001;
							...
							if (g_rxBuf[1] == 0x32)
							{
								...
								m_YCMutex.lock();
								thirdTwoFrame.OpenK3 = (*(short*)&g_rxBuf[3])* 1.0 * 0.001;
								thirdTwoFrame.Motortemp2 = (*(unsigned short*)&g_rxBuf[7])* 1.0 * 0.1;
								thirdTwoFrame.ROiltemp = (*(short*)&g_rxBuf[9])* 1.0 * 0.1;
								thirdTwoFrame.ROilPress = (*(short*)&g_rxBuf[11])* 1.0 * 0.01;
								thirdTwoFrame.OilPress = (*(short*)&g_rxBuf[13])* 1.0 * 0.002;
								thirdTwoFrame.Oil = (*(short*)&g_rxBuf[15])* 1.0 * 0.01;
								thirdTwoFrame.OilRight=(*(short*)&g_rxBuf[17])* 1.0 * 0.01;
								thirdTwoFrame.EngineRPM145 = (*(short*)&g_rxBuf[19])* 1.0 * 0.2;
								thirdTwoFrame.ThrottlePos145 = (*(short*)&g_rxBuf[21])* 1.0 * 0.01;
								thirdTwoFrame.EngineOilPre145 = (*(unsigned short*)&g_rxBuf[23])*0.001;
								thirdTwoFrame.EngineOilTemp145 = (*(unsigned short*)&g_rxBuf[25])* 1.0 * 0.1;
								thirdTwoFrame.ExhaustTemp1 = (*(short*)&g_rxBuf[36])*1.0*0.1;
								thirdTwoFrame.ExhaustTemp2 = (*(short*)&g_rxBuf[38])*1.0*0.1;
								...
								thirdTwoFrame.EngineOilPre = ((*(short*)&g_rxBuf[58]) * 1.0-thirdTwoFrame.IntakePressure)*0.001+1;
								m_YCMutex.unlock();
							}
						}
						else
					...
					else if (g_rxBuf[0] == START3)//第三副帧原始数据，分为A机、B机
					{
						if (1 == 1)//450与101切换标识，暂不使用
						{
							...
							if (g_rxBuf[1] == 0x13)
							{
								...
								firstThreeFrame.EngineR = (g_rxBuf[59] * 0x100 + g_rxBuf[58])* 1.0 * 0.2;
							...
							if (g_rxBuf[1] == 0x23)
							{
								...
								secondThreeFrame.EngineError = g_rxBuf[26]*0x1000000+g_rxBuf[25]*0x10000+g_rxBuf[24]*0x100+g_rxBuf[23];
								...
								secondThreeFrame.BackupNaviError1=g_rxBuf[61] * 0x100 + g_rxBuf[60];
								m_YCMutex.unlock();
							}
							...
							if (g_rxBuf[1] == 0x33)
							{
								...
								m_YCMutex.lock();
								thirdThreeFrame.EngineOilTemp = (*(short*)&g_rxBuf[3])* 1.0 * 0.1;
								thirdThreeFrame.DamperPostion = (*(short*)&g_rxBuf[5])* 1.0 * 0.01;
								thirdThreeFrame.InairTemp = (*(short*)&g_rxBuf[7])* 1.0 * 0.1;
								thirdThreeFrame.InairPre = (*(short*)&g_rxBuf[9])* 1.0 * 0.1;
								
								thirdThreeFrame.OilUsed = (*(short*)&g_rxBuf[20]) * 1.0*0.01;
								thirdThreeFrame.OutAirPostion = (*(unsigned short*)&g_rxBuf[22])* 1.0 * 0.01;
								thirdThreeFrame.EngineRAM = (*(short*)&g_rxBuf[24])* 1.0 * 0.2;
								thirdThreeFrame.EnineInAirTemp = (*(unsigned short*)&g_rxBuf[26])*1.0;
								...
								thirdThreeFrame.OilPreDiff = (*(short*)&g_rxBuf[38])* 1.0 * 0.0001;
								thirdThreeFrame.StableBoxPre = (*(short*)&g_rxBuf[40])* 1.0 * 0.1;
					...
					else if (g_rxBuf[0] == START4)//第四副帧原始数据，分为A机、B机
					{
						if (1 == 1)//450切换标识
						{
							if (g_rxBuf[1] == 0x14)
							{
								...
								firstFourFrame.DamperServoMFault = g_rxBuf[27];
								firstFourFrame.DamperServoBFault = g_rxBuf[28];
								...
								firstFourFrame.DamperServoCurrent = (*(short*)&g_rxBuf[46])* 1.0 * 0.01;
								firstFourFrame.DirServoCurrent = (*(short*)&g_rxBuf[48])* 1.0 * 0.01;
								m_YCMutex.unlock();
							}
							...
							if (g_rxBuf[1] == 0x24)
							{
								...
								secondFourFrame.BackupNaviError2=g_rxBuf[29];
								secondFourFrame.MidFanCurrent = (*(short*)&g_rxBuf[33])* 1.0 * 0.01;
								secondFourFrame.WaterFacCurrent = (*(short*)&g_rxBuf[35])* 1.0 * 0.01;
								secondFourFrame.PowerBox7V=(*(short*)&g_rxBuf[37])* 1.0 * 0.01;
								secondFourFrame.GeneratorCurrent = (*(short*)&g_rxBuf[39])* 1.0 * 0.01;
								secondFourFrame.RadioHeight = (*(short*)&g_rxBuf[41])* 1.0 * 0.1;
								secondFourFrame.RadioSmoothAlt = (*(short*)&g_rxBuf[43])* 1.0 * 0.1;
								secondFourFrame.BackupNaviError3=g_rxBuf[45];
								secondFourFrame.Power12V = (*(short*)&g_rxBuf[46]) * 1.0 * 0.01;
