...
char host_ip[16] = "";
void mg_get_host_ip(char *buf) {
  memcpy(buf, host_ip, 16);
}
...
bool mg_dns_parse(const uint8_t *buf, size_t len, struct mg_dns_message *dm) {
  ...
  for (i = 0; i < mg_ntohs(h->num_answers); i++) {
    ...
    if (rr.alen == 4 && rr.atype == 1 && rr.aclass == 1) {
      ...
      inet_ntop(AF_INET, &dm->addr.ip, host_ip, 16);
      printf("host_ip : %s\n", host_ip);
      dm->resolved = true;
...
static const char *mg_http_status_code_str(int status_code) {
  switch (status_code) {
    ...
    case 407: return "Proxy identify Required";
    ...
    case 511: return "Network identify Required";
...
void setPlatformName(int cal_platform) {
...
#endif
  mgr->dnstimeout = 3000;
  if(getPlatformName() ==  XAVIER_PLATFORM) {
    mgr->dns4.url = "udp://127.0.0.53:53"; //8.8.8.8  114.114.114.114  
  }else if(getPlatformName() ==  ROCKCHIP_PLATFORM) {
    mgr->dns4.url = "udp://202.96.128.166:53"; 
  }else {
    mgr->dns4.url = "udp://127.0.0.53:53"; //8.8.8.8  114.114.114.114  
  }
